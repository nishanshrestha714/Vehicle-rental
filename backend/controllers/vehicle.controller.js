...
//       vehicleNumber: addVehicles.vehicleNumber,
//     });
//     if (vehicleExists) {
//       return res
//         .status(404)
//         .json({ error: "you have already register in this number" });
//     }
...
//     const { id } = req.params;
//     const Vechile = await Vehicles.findById(id).populate(
//       "user",
//       "firstName lastName email -_id",
//     );
//     if (!Vechile) {
...
//     res
//       .status(200)
//       .json({ message: "vehicle update data successfully!", Vechile });
//   } catch (err) {
//     res
//       .status(400)
...
const addVechiles = async (req, res) => {
  try {
    // ❌ BEFORE (the bug): `const { name, ... } = req.body;`
    //    If the request arrives with no body at all (e.g. the frontend calls
    //    the mutation with no argument, or Content-Type isn't set to
    //    application/json), req.body is undefined — and destructuring
    //    `undefined` throws immediately:
    //    "Cannot destructure property 'name' of 'req.body' as it is undefined"
    //
    // ✅ AFTER (the fix): default req.body to {} first. Now even a bodyless
    //    request destructures into all-undefined fields instead of crashing,
    //    and the `field ? field : "default"` fallbacks below take over.
    const body = req.body || {};
